...
class Welcome extends CI_Controller {
   ...
   public function index(){
      ...
      $job_data = $this->callAPI('GET', 'https://api-int.monj.in/f/22/1/1/list/published-job?sort=-createdOn', $token,$strt,$limit);
   ...
   public function job_details($key){
      ...
      $strt = 1;
      $limit = 10;
      $filter_data = $this->callAPI('GET', 'https://api-int.monj.in/f/22/1/1/list/published-job?filter=code['.$key.']', $token,$strt,$limit);
      $data['response'] = json_decode($filter_data, true);
      $data_video = $this->fetch_video_src($data['response'][0]['standardvideourl']);
      $data['video_status'] = json_decode($data_video, true);
      // print_r($data['video_status']);die();
   ...
   public function fetch_count()
   {
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api-int.monj.in/f/22/1/1/list/published-job?sort=-createdOn",
   ...
   public function feed(){
      $token = $this->get_access_token();
    $sXML = $this->callAPIFeed('GET', 'https://api.monjin.com/f/22/1/1/job/Feed', $token);
    $xml = new SimpleXMLElement($sXML);
#    echo "<strong><CENTER><h2>".$xml->channel->title. "</h2></h3>" . $xml->channel->description. "</h3></CENTER></strong>";
#    print_r($xml->asXML());
#      $data['job_data'] = $this->callAPIFeed('GET', 'https://api.monjin.com/f/22/1/1/job/Feed', $token);
#      return $data;
#      $data['response'] = json_decode($job_data, true);
#      $this->load->view('feed',$data);
   ...
   public function callAPIFeed($method, $url, $data)
   {
#      $curl = curl_init();
#      switch ($method){
#         case "GET":
...
#      return $result;
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://api.monjin.com/f/22/1/1/job/Feed",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
   ...
   public function fetch_video_src($key)
   {
      $token = $this->get_access_token();
      $data = array(
        "standardVideoUrl" => "".$key.""
      );
      $payload = json_encode($data);
      $curl = curl_init();
      curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api-int.monj.in/a/22/1/1/1/vpu/resource",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_CUSTOMREQUEST => "POST",
        CURLOPT_POSTFIELDS => "[".$payload."]",
        CURLOPT_HTTPHEADER => array(
